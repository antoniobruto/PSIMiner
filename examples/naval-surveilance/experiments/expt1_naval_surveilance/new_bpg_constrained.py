...
def read_from_config(config_file_name,signal_list,other_arguments,influence_list):
	...
	for row in fptr:
		...
		if( line[0] == 'bias'):
			...
			continue
		if( line[0] == 'tschedule'):
			tschedule = line[1].strip()
			print 'tschedule = ', tschedule
			if(tschedule == 'geometric'):
				tschedule = 1
			else:
				tschedule = 0
			print 'tschedule = ', tschedule
	...
	other_arguments.append(tschedule)
...
def compute_entropy_for_current_node():
	...
	if(influence_list == []):
		temp_bucket_number = 0
	print 'while computing error for current node bucket position is',temp_bucket_number
...
def find_error_for_predicate(predicate,bucket_number):
	...
	print 'finding error for ',predicate[2],'at bucket number',bucket_number
	#interval = find_Interval(predicate)
	#finding intervals

	temp_minimum_bucket_value = min(bucket_number,initial_minimum_bucket_value)
	print 'temp_minimum bucket found is ',temp_minimum_bucket_value
...
def store_m_best_predicates(gain,predicate,bucket_number):
	...
	if gain <= 0:
		return
	store_m_best_predicates_global(gain,predicate,bucket_number)
	...
	if(len(m_best_predicates) > m):
		...
		del m_best_predicates_interval[-1]

def store_m_best_predicates_global(gain,predicate,bucket_number):
	global considered_predicate
	global considered_predicate_interval
	global m_best_predicates_global
	global m_best_predicates_interval_global
	print 'predicate and gain recieved is'
	print predicate,gain,bucket_number
	print 'current best predicates'
	print m_best_predicates_global
	if gain <= 0:
		return
	

	# store_m_best_predicates_bucket_wise(gain,predicate,bucket_number)

	for i in range(len(m_best_predicates_global)):
		item_gain,item_predicate,item_bucket_number = m_best_predicates_global[i]
		# print 'both gains are ',item_gains,gain
		# if(item_gain == gain):
		# 	print 'they are equal'
		# else:
		...
		if(item_gain == gain and item_bucket_number == bucket_number):
			...
			if(item_predicate_signalname == predicate_signalname and item_predicate_operator == predicate_operator):
				if(predicate_operator == '>=' and predicate_constant >= item_predicate_constant):
					m_best_predicates_global[i][1] = predicate
					# m_best_predicates_global[i][3] = extra_info
					m_best_predicates_interval_global[i] = considered_predicate_interval
					return
				if(predicate_operator == '<=' and predicate_constant <= item_predicate_constant):
					m_best_predicates_global[i][1] = predicate
					# m_best_predicates[i][3] = extra_info
	...
	if((gain,predicate,bucket_number) in m_best_predicates_global):
		print 'predicate already added to priority list'
		return

	index_new_predicate = 0
	len_m_best_predicates_global = len(m_best_predicates_global)
	if(len_m_best_predicates_global == 0):
		m_best_predicates_global.append((gain,predicate,bucket_number))
		# m_best_predicates.append((gain,predicate,bucket_number,extra_info))
		m_best_predicates_interval_global.append(considered_predicate_interval)
		return
	else:
		while ( index_new_predicate < len_m_best_predicates_global):
			if(gain > m_best_predicates_global[index_new_predicate][0]):
				break
			index_new_predicate+=1
	# if(index_new_predicate != m):
	m_best_predicates_global.insert(index_new_predicate,(gain,predicate,bucket_number))
	# m_best_predicates.insert(index_new_predicate,(gain,predicate,bucket_number,extra_info))
	m_best_predicates_interval_global.insert(index_new_predicate,considered_predicate_interval)
	print 'predicate =',predicate
	print 'considered predicate =', considered_predicate
	if(len(m_best_predicates_global) > m):
		del m_best_predicates_global[-1]
		del m_best_predicates_interval_global[-1]


# The func prints the top m predicates in the file as well as terminal
def print_m_best_predicates_global():
	# global m_best_predicates_global
	# global m_best_predicates_interval_global
	print 'printing m_best_predciates global'
	print m_best_predicates_global
	fileptr = open('best_predicates.txt','w')
	fileptr.truncate()

	print 'index \t gain \t\t predicate \t\t bucket_number \n',
	for i in range(0,len(m_best_predicates_global)):
		fileptr.write("begin_pred\n")
		fileptr.write(str(m_best_predicates_global[i][1][2]) + '\n' )
		fileptr.write('gain = ' + str(m_best_predicates_global[i][0]) + '\n')
		fileptr.write('bucket = ' +  str(m_best_predicates_global[i][2]) + '\n')
		fileptr.write('intervals = ' + m_best_predicates_interval_global[i])
		fileptr.write('end_pred\n\n')
		print i+1,'\t',m_best_predicates_global[i][0],'\t',m_best_predicates_global[i][1][2],'\t',m_best_predicates_global[i][2]
	fileptr.close()

def some_xyz_function(m_best_predicates_global,m_best_predicates_interval_global):
	# global m_best_predicates_global
	print m_best_predicates_global
	print m_best_predicates_interval_global
...
def print_m_best_predicates_bucket_number(bucket_number):
	filename = 'bucket_'+str(bucket_number)+'_best_predicate.txt'
...
def clear_m_best_predicate_bucket_wise(bucket_number):
	global m_best_predicates
	global m_best_predicates_interval
	print_m_best_predicates_bucket_number(bucket_number)
	m_best_predicates = []
	m_best_predicates_interval = []

def sa_old_archive_code():
	#local variables 
	max_gain = 0.0
	...
	# error_temp = find_error_for_predicate((2,1,'x >= 50'),25)
	# print 'current error of node is ',curr_error
	# print 'error for predicate is ',error_temp
	# print 'exiting'
	# sys.exit()
	...
	for bucket_number in range(n+1):
		...
		clear_m_best_predicate(bucket_number)
	print 'Bucket number ',bucket_number,' finished'
...
def sa():
	...
	# predicate_temp = (2,1,'x >= 35')
	# for bn in range(n+1):
	# 	err = find_error_for_predicate(predicate_temp,bn)

	# 	print 'at position',bn,' the gain is',(curr_error - err)
	# 	print '\n\n\n'
	...
	for bucket_number in range(n+1):
		...
		clear_m_best_predicate_bucket_wise(bucket_number)
	print 'Bucket number ',bucket_number,' finished'
...
if __name__ == "__main__":
		...
		global m_best_predicates_global
		m_best_predicates_global = []

		global m_best_predicates_interval
		m_best_predicates_interval = []

		global m_best_predicates_interval_global
		m_best_predicates_interval_global = []
		...
		global tschedule
		tschedule = 0
		if(len(other_arguments) >= 10):
			tschedule = other_arguments[9]
		...
		sa()
		# some_xyz_function(m_best_predicates_global,m_best_predicates_interval_global)
		print_m_best_predicates_global()